/*
  # Fix Dashboard OS Metrics Regression
  
  ## Summary
  Corrects the `get_dashboard_consolidado` function to use the proper property names 
  and status values, fixing the issue where "OS Não Entregues" was showing no value.

  ## Changes
  - Renames `os_abertas` to `os_nao_entregues` in the JSON response.
  - Updates status filters to use exact matches: "Não Entregue" and "Entregue".
*/
...
BEGIN
  ...
  -- Get OS summary - FIX: Property names and status values
  ...
  -- Ranking Descontos por Vendedor
  ...
  -- Ranking Recorrência de Clientes
