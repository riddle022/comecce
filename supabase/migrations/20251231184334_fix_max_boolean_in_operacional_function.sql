/*
  # Fix MAX(boolean) Error in Operacional Function

  ## Summary
  Fixes the error "function max(boolean) does not exist" by replacing MAX(em_atraso) 
  with BOOL_OR(em_atraso) which is the correct PostgreSQL function for boolean aggregation.

  ## Changes
  - Replace MAX(em_atraso) with BOOL_OR(em_atraso) in the os_aggregated CTE
  - BOOL_OR returns true if any value in the group is true, which is the desired behavior
*/
...
-- Add comment
COMMENT ON FUNCTION get_operacional_os_summary IS 'Returns operational OS summary with server-side pagination, filtering, and sorting. KPIs are calculated based on filtered data. Uses BOOL_OR for boolean aggregation.';
