/*
  # Fix CTE Scope Error in Operacional Function

  ## Summary
  Fixes the error "relation 'os_filtered_final' does not exist" by reorganizing 
  the query to keep all CTE references within a single query context.

  ## Changes
  - Consolidate all CTE usage into a single query block
  - Calculate KPIs and fetch paginated data in one operation
  - Use temporary table to store filtered results for reuse
*/
...
-- Add comment
COMMENT ON FUNCTION get_operacional_os_summary IS 'Returns operational OS summary with server-side pagination, filtering, and sorting. All CTEs are within a single query context.';
