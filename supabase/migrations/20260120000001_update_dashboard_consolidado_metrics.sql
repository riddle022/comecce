/*
  # Update Dashboard Consolidated Metrics
  
  ## Summary
  Updates `get_dashboard_consolidado` to include detailed daily metrics in the `tendencia` array.
  This allows the frontend to perform accurate monthly aggregations for the "Faturamento" table.

  ## Changes
  - Adds `quantidade_vendida`, `desconto_total`, and `numero_vendas` to the `daily_totals` CTE.
  - Returns these fields in the `tendencia` JSON array.
*/
...
BEGIN
  ...
  -- Get daily trend - UPDATED TO INCLUDE DETAILED METRICS
  WITH daily_totals AS (
    SELECT
      ...
      COALESCE(SUM(item_desconto_total), 0) as desconto_total,
      COALESCE(SUM(item_vl_custo_unitario * item_nr_quantidade), 0) as custo_total,
      COUNT(DISTINCT numero_venda) as numero_vendas
    FROM tbl_vendas
    WHERE
      DATE(data_venda) BETWEEN p_data_inicio AND p_data_fim
      AND id_empresa = ANY(p_empresa_ids)
  ...
  -- Ranking Descontos por Vendedor (Top 5)
  ...
  -- Ranking Recorrência de Clientes (Clientes com > 1 compra)
